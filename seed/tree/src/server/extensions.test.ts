...
describe("reading the graph's extension namespace", () => {
  ...
  it("Given the recorded skills, Then the two skill-only extensions each hold one", () => {
    ...
    expect(
      grouped.get("node:calliopa-base")?.[0]?.revision.content["goal"],
    ).toMatch(/proposal/u);
  });
  ...
  it("Given the recorded pin and a manifest, Then the summary says whether newer truth waits", () => {
    ...
    if (!manifests.ok || !sources.ok || !skills.ok || !pins.ok)
      throw new Error("fixtures did not parse");
    const pin = nodesOfType(pins.value, "kernel.releasepin")[0]?.revision
      .content["pin"];
    expect(pin).toBe(17);
    const snapshot = {
      head: 20,
      ...
      servedPin: 17,
      elevated: ["ui.shell"],
      inactive: ["calliopa-extension"],
      pins: new Map([["ui.shell", 12]]),
      declaring: new Set<string>(),
    };
    const shell = snapshot.manifests.find(
      (node) => node.id === "node:ui.shell",
    );
    const extension = snapshot.manifests.find(
      (node) => node.id === "node:calliopa-extension",
    );
    if (shell === undefined || extension === undefined)
      throw new Error("manifests missing");
    // The shell's newest member revision is the README round trip at 17: current.
    expect(forTesting.summaryOf(snapshot, shell)).toMatchObject({
      id: "ui.shell",
      category: "bundled",
      ahead: false,
      active: true,
    ...
    expect(forTesting.summaryOf(snapshot, extension)).toMatchObject({
      id: "calliopa-extension",
      ahead: true,
      newestRevision: 20,
      active: false,
      pinned: false,
  ...
  it("Given a refusal or a non-JSON answer, Then the reason is reported rather than thrown", () => {
    expect(parseQueryAnswer("not json")).toEqual({
      ok: false,
      detail: "The gateway's answer was not JSON.",
    });
    const refused = parseQueryAnswer(
      JSON.stringify({
        status: "error",
        error: { message: "unknown_node_type" },
      }),
    );
  ...
  it("Given members, Then only docs and the README are read as owner docs", () => {
    ...
    const docs = forTesting.docsOf(
      membersByManifest(reply.value).get("node:ui.shell") ?? [],
    );
...
describe("an extension's history", () => {
  it("Given metadata-only history of its members, Then superseded revisions still name the proposal that wrote them", async () => {
    ...
    if (!skills.ok || !manifests.ok || !groups.ok)
      throw new Error("fixtures did not parse");
    const events =
      eventsByManifest(skills.value, manifests.value).get(
        "node:calliopa-extension",
      ) ?? [];
    ...
    expect(rationales.some((text) => text.startsWith("BO_0201_001"))).toBe(
      true,
    );
    expect(rationales.some((text) => text.startsWith("BO_0200_016"))).toBe(
      true,
    );
    expect(rationales.some((text) => text.startsWith("BO_0200_015"))).toBe(
      true,
    );
    const proposals = nodesOfType(groups.value, "ProposalGroup").map(
      (node) => ({
        id: node.id,
    ...
    expect(
      history.find((entry) => entry.rationale.startsWith("BO_0200_016")),
    ).toMatchObject({ status: "accepted", dataRevision: 20 });
    expect(
      history.find((entry) => entry.rationale.startsWith("BO_0200_015")),
    ).toMatchObject({ status: "accepted", dataRevision: 12 });
