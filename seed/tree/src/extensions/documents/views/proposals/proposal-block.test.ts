...
describe("work items in the block editor", () => {
  ...
  it("Given a possible relation, Then it is drawn under its block with its claim, kind, target and reason, and the reason is editable", async () => {
    ...
    expect(text).toContain("Possible relation");
    expect(text).toContain("Inferred");
    expect(text).toContain("Revision changes can invalidate a cached result.");
    expect(text).toContain("qualifies");
    expect(text).toContain("Requests observe one stable revision.");
    expect(text).toContain("Request semantics");
    const reason = card?.querySelector("[data-proposal-reason]");
    expect(reason?.getAttribute("contenteditable")).toBe("true");
    expect(reason?.textContent).toBe("revision advancement mid-request is untested");
    const rows = Array.from(view.root.querySelectorAll("[data-block-id], [data-proposal-id]")).map(
      (row) => row.getAttribute("data-proposal-id") ?? row.getAttribute("data-block-id"),
    );
    expect(rows.indexOf(relate)).toBe(rows.indexOf("blk-b") + 1);
    // No face drags a relation: it has no place of its own.
    expect(card?.querySelector("[data-proposal-face]")?.getAttribute("aria-label")).toBe("Proposed by Claude Code (claude-sonnet-5)");
    await view.settle();
  });
  ...
  it("Given a relation whose source is in another document, Then it is counted and not drawn", async () => {
    const view = await mountWork();
  ...
  it("Given the reason edited and left, Then the relation is accepted as an edit and its reason revised on top", { timeout: 10000 }, async () => {
    const view = await mountWork();
    const reason = view.proposal(relate)?.querySelector("[data-proposal-reason]") as HTMLElement;
    reason.textContent = "cache reuse assumes one stable authorization context";
    await view.userEvent(reason, "input");
    await view.userEvent(reason, "blur");
    await view.waitFor(() => view.commands("reviseReason").length === 1);
    const answered = view.commands("answerProposal");
    expect(answered[0]?.body).toMatchObject({ itemId: relate, answer: "accepted", edited: true });
    expect(view.commands("reviseReason")[0]?.body).toMatchObject({
      relationId: "rel-1",
      baseRevisionId: "rev-rel-1",
      reason: [{ text: "cache reuse assumes one stable authorization context" }],
    });
    await view.settle();
  });
