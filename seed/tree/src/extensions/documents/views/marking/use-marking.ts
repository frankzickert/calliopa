...
export function useMarking(input: {
  ...
  const { documentId, surface, leaveEditing$, beforeReference$ } = input;
  ...
  const toggleReference$ = $((blockId: string) => {
    const marked = !store.marking.references.some((reference) => reference.blockId === blockId);
    ...
    if (marked && beforeReference$ !== undefined) void beforeReference$(blockId);
  });
