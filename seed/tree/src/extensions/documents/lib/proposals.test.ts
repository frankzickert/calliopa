...
describe("a proposals list narrowed by what was answered", () => {
  ...
  it("names the kernel's own run as the system, with the agent it reasoned on beside", () => {
    const proposer = proposerOf({ ...claudeRun, trigger: "system" }, ["agent:hermes"]);
    expect(proposer).toEqual({ kind: "system", agent: "claude-code", executedBy: claudeRun["executedBy"] });
    expect(proposerName(proposer)).toBe("the system, as Claude Code");
    expect(toneOf(proposer)).toBe("system");
    expect(faceOf(proposer)).toEqual({ kind: "icon", icon: "sparkle" });
  });
