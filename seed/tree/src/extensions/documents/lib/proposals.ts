...
export type ProposalTone = "codex" | "claude" | "hermes" | "person" | "system";

export function toneOf(proposer: Proposer): ProposalTone {
  if (proposer.kind === "person") return "person";
  if (proposer.kind === "system") return "system";
...
export type ProposalFace =
  ...
  | { readonly kind: "icon"; readonly icon: "user" | "robot" | "sparkle" };

export function faceOf(proposer: Proposer): ProposalFace {
  if (proposer.kind === "person") return { kind: "icon", icon: "user" };
  if (proposer.kind === "system") return { kind: "icon", icon: "sparkle" };
  switch (proposer.agent) {
    case "codex":
...
export function proposerName(proposer: Proposer): string {
  ...
  if (proposer.kind === "system") {
    const on = proposer.agent === null ? "" : `, as ${AGENT_NAMES[proposer.agent] ?? proposer.agent}`;
    return `the system${on}`;
  }
