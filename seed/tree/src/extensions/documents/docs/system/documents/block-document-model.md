...
- [ ] BO_0296_013 `document` permits `formatCode`, a flag defaulting to on, staged as a member revision through `kernel commit --members` as `runtime` was; `validateDocumentProperties` says in this extension's words what it carries, and it is written through the depth as a person's own edit, as the `runtime` connection is.
- [ ] BO_0296_014 The read colours a code block (`server/assemble.ts`): `CodeBlockView` gains the coloured markup, set as the assembler sets an equation's, from the block's `language`; a block with no language, a language the highlighter does not know and a source it refuses answer no markup at all, and the view draws the plain characters it draws today. Proven in `server/code-content.test.ts`: a Python block coloured, an unknown language answered plain, and a block whose source is not valid in its language still coloured as far as the grammar reaches, never refused — a document is full of code that does not compile.
- [ ] BO_0296_015 `reviseCode` formats before it writes (`server/documents.ts`): with the document's `formatCode` on, the source goes to `POST /__kernel/code/format` with its language and the answer is what the revision carries; an answer of *unchanged*, a refusal, a timeout and an unreachable kernel each write the source exactly as typed, so a settled edit is never lost to a service being down, and nothing is said to the person. With `formatCode` off, nothing is called. Proven against a stub of the route for each of those outcomes.
- [ ] BO_0296_016 Acceptance formats (`server/documents.ts`, `answerDocumentProposal`): accepting a proposal that carries a `sourcecode` block, in a document whose `formatCode` is on, records the acceptance as it does today and then writes one further revision of that block carrying the formatted source, authored by the accepter; the staged revision is never rewritten. A block whose formatted source equals what was proposed writes no second revision. Proven on a proposal holding an unformatted code block, and on one where formatting is off.
...
- [ ] BO_0296_017 A language is guessed once where a block is created (`NewCodeBlock` and `turnIntoCode`): a block created with no language gets the highlighter's guess written into `language` as an ordinary value a person can change, and nothing re-guesses it afterwards; a block a person empties the language of stays empty and draws plain. A guess the highlighter will not make leaves the field empty.
