...
import { emitClient, emitServer, RegistryScanError, scanExtensions, writeRegistry } from "../scripts/registry.mjs";
  ...
  for (const [name, fixture] of Object.entries(extensions)) {
    ...
    if (fixture.manifest !== undefined) {
      writeFileSync(
        ...
        typeof fixture.manifest === "string" ? fixture.manifest : JSON.stringify(fixture.manifest),
      );
    }
    if (fixture.client !== undefined) writeFileSync(join(dir, "contributions.ts"), fixture.client);
    if (fixture.server !== undefined) writeFileSync(join(dir, "contributions.server.ts"), fixture.server);
  }
  return root;
}
...
describe("scanning the extensions present", () => {
  it("finds every extension, with the halves of its entrypoint that exist", () => {
    const root = tree({
      "ui.shell": { manifest: manifest("ui.shell", { entrypoint: "contributions" }), client: CLIENT, server: SERVER },
      settings: {
        manifest: manifest("settings", { entrypoint: "contributions", dependencies: { "ui.shell": ">=0.1.0" } }),
        client: CLIENT,
      },
      "calliopa-base": { manifest: manifest("calliopa-base") },
    });
    const entries = scanExtensions(root);
    expect(entries.map((entry) => entry.id)).toEqual(["calliopa-base", "settings", "ui.shell"]);
    expect(entries.map((entry) => [entry.client !== null, entry.server !== null])).toEqual([
      [false, false],
      [true, false],
      [true, true],
    ]);
  ...
  it("emits one module per half importing only the halves present", () => {
    const root = tree({
      "ui.shell": { manifest: manifest("ui.shell", { entrypoint: "contributions" }), client: CLIENT, server: SERVER },
      settings: { manifest: manifest("settings", { entrypoint: "contributions" }), client: CLIENT },
    });
    const entries = scanExtensions(root);
    ...
    expect(server).toContain('from "~/extensions/ui.shell/contributions.server"');
  ...
  it("is the same build without an extension: its entry is simply absent", () => {
    const without = scanExtensions(
      tree({ "ui.shell": { manifest: manifest("ui.shell", { entrypoint: "contributions" }), client: CLIENT } }),
    );
    const client = emitClient(without);
    expect(client).toContain('from "~/extensions/ui.shell/contributions"');
    expect(client).not.toContain("settings");
    expect(client).toContain('PRESENT_EXTENSIONS: readonly string[] = ["ui.shell"]');
...
describe("what fails the build by name", () => {
  it("a directory without a manifest", () => {
    expect(code(() => scanExtensions(tree({ stray: { client: CLIENT } })))).toBe("manifest_missing");
  });

  it("an id not matching its directory", () => {
    expect(code(() => scanExtensions(tree({ settings: { manifest: manifest("setting") } })))).toBe("id_mismatch");
  });

  it("an entrypoint naming no module", () => {
    expect(code(() => scanExtensions(tree({ a: { manifest: manifest("a", { entrypoint: "contributions" }) } })))).toBe(
      "entrypoint_missing",
    );
  });
  ...
  it("an entrypoint that does not export the contract's shape", () => {
    expect(
      code(() =>
        scanExtensions(tree({ a: { manifest: manifest("a", { entrypoint: "contributions" }), client: "export const x = 1;\n" } })),
      ),
    ).toBe("entrypoint_shape");
    expect(
      code(() =>
        scanExtensions(
          tree({ a: { manifest: manifest("a", { entrypoint: "contributions" }), client: CLIENT, server: "export default {};\n" } }),
        ),
      ),
    ).toBe("entrypoint_shape");
    expect(code(() => scanExtensions(tree({ a: { manifest: manifest("a", { entrypoint: "../x" }) } })))).toBe(
      "entrypoint_shape",
    );
  });
  ...
  it("a declared dependency the tree does not hold", () => {
    expect(
      code(() => scanExtensions(tree({ a: { manifest: manifest("a", { dependencies: { "ui.shell": ">=0.1.0" } }) } }))),
    ).toBe("dependency_missing");
  });

  it("a manifest that is not JSON", () => {
    expect(code(() => scanExtensions(tree({ a: { manifest: "{not json" } })))).toBe("manifest_unreadable");
  });
});
