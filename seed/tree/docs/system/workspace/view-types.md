...
- `setPointing$` records what the reader has marked in a target, in mark order, so a command from the composer can carry it; the view names its target for the reason `setSaveState$` names its tab. `proposed` is the other direction: a store the shell writes as `{itemId, seq}` when a run aimed at a target ends, `seq` rising so a second run against the same target is told from the first — `ViewDrop`'s shape. What the run staged is the view's to read; the shell only says there is something to read. A view starts having seen the count that stood when it opened (`proposedFor` in `src/lib/command-target.ts`), so a run that ended before the reader opened the target is not acted on again (`BO_0226_006`, `BO_0226_007`).
...
* The view bridge carries the workspace the view is mounted in, `workspaceId`, read-only, so a contributed view can create a process for it through the shell's own registry the way the composer's run does; a view never changes the workspace it is in. Requested by `PU_0009` in `publishing`, 2026-09-15 (`CA_0050`).
...
- `setPointing$` carries a `Pointing` since `BO_0227`: the references in mark order — a block, or a passage with its quote — each with the words the composer shows and whether a passage is stale, and the blocks pinned in the target. What travels to a run is the references alone (`SentReference`, `src/lib/command-target.ts`). `offerUndo$` hands the shell an `UndoOffer` — the words for the dock's undo line and the view's own inverse — which a tab switch clears (`BO_0227_013`, `BO_0227_015`).
