...
- [Workspace Shell](./frame.md) owns tabs, drawers, themes, drag coordination, and process reporting. This document owns only the boundary where a tool mounts inside a tab.
...
* A view type declares its inspector contribution, its bar and its drag capabilities. The shell reads those declarations; a view never takes ownership of a shell surface. The bar is a shell contract rather than a view's own markup: user decision, 2026-09-18 (`CA_0053`).
...
- The contract carries one thing the other way since `CA_0065_004` (2026-09-23): the shell contributes a `BlockControl` into a view's own block row, which the view renders through `ActionControl` at the third `ActionSurface`, `"block"`, in the one named-action vocabulary. A view's extension contributes how a child of its target kind is made, so the shell can open any block as focused work without writing a vocabulary it does not own. Both, the bridge calls that go with them and what a view may not do with a control it is handed are [Focused Work](./focused-work.md); nothing about them is restated here.
...
- A view contributes a bar beside its inspector contribution: an ordered list of groups, each with an identifier, an accessible name, its controls in the named-action vocabulary, and whether it is the trailing group. The view writes it to the bridge's `bar` store; the shell draws it ([Layout](./layout.md#the-view-bar)) and clears it on a tab switch, as it clears the inspector's contribution. A view that contributes no group gets no bar, and a view that contributes one keeps its content clear of `--view-bar-height` at its top. Only the document view contributes one so far (`CA_0053_001`).
- The named-action vocabulary carries what the document view's bar needed, and the bar and the inspector render every shape through one control, because the vocabulary is one: an optional `icon` on `button` and `toggle`, shown in place of the label, which stays the accessible name and the tooltip; an optional `name`, the accessible name where it says more than the label shown; `disabled` on a button that is genuinely unavailable; `keepsSelection`, by which a control keeps the text selection it acts on — the shell renders `preventdefault:mousedown`, so the press takes no focus; and `field`, a line of text the reader types and applies — its value, its label, its `type`, `input$` for every keystroke and `submit$` for its button and Enter. The standing scale is a `choice`, which carries all it shows (`CA_0053_002`).
...
- `setPointing$` records what the reader has marked in a target, in mark order, so a command from one of its blocks can carry it; the view names its target for the reason `setSaveState$` names its tab. `proposed` is the other direction: a store the shell writes as `{itemId, seq}` when a run aimed at a target ends, `seq` rising so a second run against the same target is told from the first — `ViewDrop`'s shape. What the run staged is the view's to read; the shell only says there is something to read. A view starts having seen the count that stood when it opened (`proposedFor` in `src/lib/command-target.ts`), so a run that ended before the reader opened the target is not acted on again (`BO_0226_006`, `BO_0226_007`).
- `reveal` is a second store the shell writes and the view reads, shaped as `proposed` is: `{itemId, target, seq}`, where `target` is a block — for a block reference or a pinned block — or a passage by its number in its block (`RevealTarget`, `revealTarget` in `src/lib/command-target.ts`), and `seq` rises with every press, so a second press on the same chip reveals again. A view acts on it through `revealFor`, `proposedFor`'s rule: a press made before the view opened is not acted on, and one for another target is seen and passed over. What revealing means is the view's; the shell only says what the reader asked to see (`CA_0039_004`).
...
- `retarget$` is the bridge call by which a view moves its own tab to another document — opening a block as focused work, or going back along the route (`CA_0047_004`): it names the target's id and title, the route the tab now carries, and optionally a block to land on; the shell rewrites the active tab through `retargetTab` (`src/lib/tabs.ts`), saves the workspace record, and remounts the view, keyed by the tab and its target. `focus` is the store beside `reveal` the shell writes for that landing — `{itemId, blockId, seq}` — and the mounted view reads once its document shows, clearing the block so a later remount of the same tab does not land there again; it never reaches the workspace record ([Block Editor](../documents/block-editor.md)).
- `ViewInspector` in `src/components/shell/view-bridge.ts` carries typed facts and named actions beside the fallback text. `InspectorFact` is a save state, a time, a count, or a line of text; `ViewAction` is a button, a toggle, a choice or a field carrying its own handler, and is the one named-action vocabulary, rendered through `ActionControl` in `src/components/shell/inspector.tsx` by the inspector and by the bar; each surface marks its controls with its own attribute — `data-inspector-action`, `data-bar-action` (`CA_0053_002`). A view that declares neither still falls back to its registry line, which is what the placeholder views contribute (`CA_0015_004`).
* The view bridge carries the workspace the view is mounted in, `workspaceId`, read-only, so a contributed view can create a process for it through the shell's own registry the way a command's run does; a view never changes the workspace it is in. Requested by `PU_0009` in `publishing`, 2026-09-15 (`CA_0050`).
- `ViewBridge.workspaceId` in `src/components/shell/view-bridge.ts` is set where the shell builds the bridge in `shell.tsx` from the workspace it holds; the editor harness (`src/components/views/testing/editor-harness.ts`) hands its views `harness-workspace`, and `view-bridge.test.ts` reads it back through a view mounted under the harness's provider (`CA_0050_001`).
...
- `setPointing$` carries a `Pointing` since `BO_0227`: the references in mark order — a block, or a passage with its quote — marked from the prompt block being edited (`BO_0267_013`), each with the words its chips show and whether a passage is stale, and the blocks pinned in the target. What travels to a run is the references alone (`SentReference`, `src/lib/command-target.ts`). A view that means to take something back names a control in its own bar, as the editor's *Take back* does (`CA_0058_011`); the shell renders no undo line (`BO_0227_015`).
